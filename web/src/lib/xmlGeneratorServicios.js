...
export const generateServicioXML = (comprobante, farmacia) => {
    ...
    const clienteTipoDoc = comprobante.tipo_documento_cliente === 'RUC' ? '6' : '1';
    ...
    comprobante.items.forEach((item, index) => {
        ...
        // IMPORTANTE: unitCode="ZZ" para servicios según SUNAT
        xml.push(`        <cbc:InvoicedQuantity unitCode="ZZ">${item.cantidad}</cbc:InvoicedQuantity>`);
        ...
        xml.push(`                <cbc:ID>${item.codigo_servicio || item.servicioId}</cbc:ID>`);
        xml.push('            </cac:SellersItemIdentification>');
        xml.push('        </cac:Item>');
        xml.push('        <cac:Price>');
    ...
    a.href = url;
    a.download = `${farmacia.ruc}-${tipoComprobante}-SERV-${numeroComprobante}.xml`;
